...
| F-11 | Notebooks Surface | 2 | P3 | 已落地基础能力 | 7-10d | Notebook 增强 |
...
- **补充能力**：F-11 基础能力已落地，后续按用户反馈继续增强导出、富 Markdown 与运行结果持久化。
...
### M1 出口
- [x] F-1、F-2、F-3 基础能力已落地，并纳入 vitest / smoke 覆盖。
- [x] shell 集成与块解析已有单元覆盖，终端主流程由 `npm run smoke:terminal` 验证。
- [x] 关闭 shell 集成时无回归（既有 smoke 测试 `npm run smoke:terminal` 通过）。

### M2 出口
- [x] F-4、F-5、F-6 基础能力已落地，并纳入 vitest / smoke 覆盖。
- [x] 现代输入编辑器与常规 terminal 输入路径无回归。
- [x] `wmux.json` 的 v0.x 配置（含 `command` 字段）保持兼容。

### M3 出口
- [x] F-7、F-8、F-9、F-10 基础能力已落地，并纳入 vitest / smoke 覆盖。
- [x] AI 凭据加密落盘；未配置 AI 时不会隐式联网。
- [x] 主题切换不需要重启应用。
